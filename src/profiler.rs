...
pub fn run_profiler() {
    ...
    total_time += profile(|| { Body::intersection(&fighter_a, &fighter_b); }, "fighters crossing without intersection");
    total_time += profile(|| { Body::intersection(&fighter_a, &fighter_c); }, "fighters side-by-side with a lot of intersections");
    total_time += profile(|| { Body::intersection(&fighter_a, &fighter_a); }, "fighters exactly at the same position");
    total_time += profile(|| { Body::intersection(&fighter_a, &fighter_d); }, "fighters slightly colliding");
